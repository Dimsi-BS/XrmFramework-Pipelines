...
$webresourcesProjectPath = Get-VstsInput -Name WebresourcesProjectPath -Require
...
if ($deployProjectPath -ne '') {
    ...
    Write-Host "##[command]$($exeFile.FullName) -n -p \"$webresourcesProjectPath\""
    . $exeFile.FullName -n -p "$webresourcesProjectPath"
